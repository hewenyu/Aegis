...
func main() {
	...
	ollamaProvider := llm.NewOllamaProvider(llm.DefaultOllamaConfig())
	...
	// 3. 创建LLM嵌入器
	embedder := llm.NewLLMEmbedder(llmService, "ollama", embedModel, 0)
	...
	searchResults, err := vectorStore.Search(ctx, defaultCollection, "向量数据库用来做什么的？", 2)
	if err != nil {
